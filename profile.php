...
<br>
    ...
    <img src="images/4565.jpg" id="reg">
    <div class="row">
...
<?php
 ...
 $sql = "SELECT * FROM users U, user_info I where U.username='$user' AND U.user_id=I.user_id";
 $result = $conn->query($sql);
if ($result->num_rows > 0) {

while($row = $result->fetch_assoc()) {
    echo"الاسم :".$row["user_fullname"]."<br>";
    echo"اسم المستخدم:".$row["username"]."<br>";
    echo"رقم الهوية الوطنية:".$row["user_nat_id"]."<br>";
    echo"الجنسية:".$row["user_nat"]."<br>";
    echo"البريد الإلكتروني:".$row["email"]."<br>";
    echo"رقم الجوال:".$row["phone"]."<br>";
    echo"تاريخ الميلاد:".$row["user_DOP"]."<br>";
    
    
    
}}else{echo"فضلا ادخل بياناتك الشخصية";}
...
while($row = $result->fetch_assoc()) {
    ...
    echo'<input type="text" name="user_id" value="'.$row['user_id'].'" hidden>';
 ...
 ?>
        <lable>اسمك الكامل</lable><input type="text" name="user_fullname" required>
        <lable>رقم الهوية الوطنية</lable><input type="text" name="user_nat_id" required>
        <lable>تاريخ الميلاد</lable><input type="date" name="user_DOP" dir="rtl" required>
        <lable>الجنسية</lable><select name="user_nat" required>
   ...
   <input type="submit" name="addpd" value="حدث بياناتك الشخصية" class="btn btn-primary btn-lg btn-block">
   <?php
    
    if(isset($_POST["addpd"])){
        $user_id= mysqli_real_escape_string($conn,$_POST['user_id']);
        $user_fullname= mysqli_real_escape_string($conn,$_POST['user_fullname']);
        $user_nat_id= mysqli_real_escape_string($conn,$_POST['user_nat_id']);
        $user_DOP= mysqli_real_escape_string($conn,$_POST['user_DOP']);
        $user_nat= mysqli_real_escape_string($conn,$_POST['user_nat']);
       
       
        $sql="INSERT INTO user_info (user_id, user_fullname, user_nat_id,user_DOP, user_nat) VALUES('$user_id',
        '$user_fullname','$user_nat_id','$user_DOP','$user_nat')";
        $result =mysqli_query($conn, $sql);
        if($result){
            echo"تم اضافة بياناتك بنجاح";
        }else{
            echo"لم يتم تسجيل بياناتك لوجود خطأ ما";
        }
    }
    ?></div>
</body>
